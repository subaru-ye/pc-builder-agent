...
> 数据发布管道已具备采集运行、字段证据、隔离与不可变 release；本机定时任务已于 2026-09-21 移除。强制约束见[数据规则](../data/数据获取与发布规则.md)，来源决策见[来源决策](../data/数据来源决策.md)，未完成工作见[路线图](../product/路线图.md)。

## 1. 设计结论

数据管道 采用“人工启动、文件留痕、条件发布、数据库投影”的方案。采集由维护者在可见终端显式执行；低风险且证据完整的变化可以自动发布，高风险变化进入隔离区。任何失败都保留上一份可用数据，不以空值、旧页面误解析或低质量来源覆盖正式数据。

本机定时任务已于 2026-09-21 移除：

- Windows 计划任务、`pcdata scheduled-run` 与 `scripts/data/ops/` 包装脚本已删除；历史 run manifest、release 和数据卷保留。
- 价格与新 SKU 的实际更新早已改走 `cmd/importprices`、`collection-tools/` 和显式 `price publish` 留痕流程，定时链自 2026-09-02 起连续失败且无人补跑。
- 项目不要求域名、服务器或 24 小时在线。以后确需周期采集，再把同一 CLI 搬到 NAS、服务器或受控 CI runner；GitHub Actions 无法直接更新只存在于本机 Docker 的数据库，不能当调度器。

数据发布主链禁止调用大模型。模型只允许在人工显式启动的离线候选辅助中使用，不能作为采集依赖、事实来源或发布裁决者。
...
- `pcdata source/collect/normalize/review/publish/bootstrap/health` CLI。
...
明确边界：数据管道 只维护 AMD CPU 既有 SKU 的规格与证据，不发现新 SKU、不采集价格；NVIDIA 自动页面采集因其条款限制而排除。

操作方式见[运行手册](../ops/本地运行与部署.md)与[价格任务](../ops/价格任务.md)。

```mermaid
flowchart LR
    A[人工显式运行] --> B[source check]
...
- 不让大模型自主周期浏览、定时抽取或直接写正式数据。
- 不在 数据管道 引入 Intel 平台、新品类或公共产品 API 变化。
...
| `pcdata.pcpart.fetch_snapshot` 下载八类 JSONL | 保留为锁定版本候选适配器；主链只检查版本，不静默升级 lock |
...
| `cmd/importprices` | 价格管道 保持旧 CSV 兼容，并导入价格观察选出的不可变快照 |
| `cmd/embedparts` 全量重算 | 增量数据维护 增加指纹后支持增量；仍保留显式全量模式 |

数据管道 不复制 canonical schema，也不让 Python 和 Go 各维护一套关键字段定义。若运行时需要目录状态，先做数据库迁移，再同步 Python 产物信封和 Go 严格解码器。
...
数据主链只接受 `method=deterministic`。人工运行可以产生 `model_assisted` 候选，但必须进入 quarantine，不能由同一次运行自动晋级。证据摘要只保留核验所需的最短文本。
...
6. 价格变化在允许范围内且商品变体完全一致；自动价格来源未激活时价格不参与网络自动更新。
...
- 本机 `var/data/checkpoints` 文件是单机恢复权威；`source_checkpoints` 仅同步 `last_attempt_at`、`last_success_at` 和状态，作为数据库可观测投影。
- 同一工作区的运行由 `var/data/scheduler/pipeline.lock` 独占文件锁串行化；发布再使用 PostgreSQL advisory lock，双重防止并发。
- `run_id` 由调用方提供或随机生成；发布幂等来自确定性 `release_id`（由 `run_id`、输入哈希和发布策略派生），同一 `run_id` 与同一输入重复发布返回同一 release，输入漂移则报 `review_drift`。
- 运行失败或超时只标记该 run，不切换 release；`pending-activation.json` 记录"数据库已写、`current.json` 未切换"的中间态，下次 bootstrap 或 publish 先恢复它。
- 人工运行前需自行确认 Docker Compose 的 `postgres` 已就绪并先执行迁移；`redis` 仅用于可选通知，不阻塞数据发布主链。

## 8. 数据库演进建议

数据管道通过编号迁移维护以下结构，但不修改产品 API DTO：
...
- 公共 API 不直接暴露证据、运维任务或发布表内部字段。
- 价格管道已增加 `price_observations`，把原始报价与最终不可变价格快照分开。
...
浏览器只用于开发适配器时观察页面结构、处理一次性人工例外和核验证据，不作为无人值守生产采集器。模型搜索结果不是来源；数据主链的网络范围由 registry 中已批准的适配器严格限定。
...
- 采集与发布主链断言 `model_used=false`，并使用 fake model guard 确认零调用。
