"""Index existing evaluation assets and summarize the frozen catalog, offline."""
import json
import sys
from collections import Counter
from decimal import Decimal

from build_current_123 import ROOT, OUT, FREEZE, SNAPSHOT_SHA, sha, write
...
def main():
    ...
    prices = {p["sku"]: Decimal(str(p["price_cny"])) for p in data["prices"] if p["snapshot_id"] == 9}
    ...
    summary = dict(active_parts=123, retired_parts=52, snapshot_id=9, snapshot_date="2026-09-15",
                   parts_release="e04a9a3c-d9bd-59a7-86d3-50ec11e21e9c", categories=dict(Counter(p["category"] for p in active)),
    ...
    for name in ["mechanisms", "screening-recorded", "thermal-recorded"]:
        ...
        entries.append(dict(id=name, path=path.relative_to(ROOT).as_posix(), sha256=sha(path),
                            ...
                            steps=sum(len(c["steps"]) for c in suite["cases"]), active_parts=123,
    ...
    if cpu.exists():
        entries.append(dict(id="cpu-recorded", path=cpu.relative_to(ROOT).as_posix(), sha256=sha(cpu),
                            cases=["LIVE123-CPU"], active_parts=123, mode="recorded_responses", is_live_score=False,
                            source="live-recording/provenance.json; unchanged first-case answers and expectations"))
    ...
    for c in manifest["cases"]:
        ...
        mapped.append(dict(id=c["id"], original_file=c["file"], original_sha256=c["sha256"],
                           ...
                           status="Historical input migrated; not a full live re-evaluation on the current 123-part catalog."))
    write(OUT / "index.json", dict(version="current-123-20260915-r2", default_suite=entries[0]["path"], suites=entries,
