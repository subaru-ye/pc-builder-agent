...
数据为空时执行 `uv run --project scripts/data pcdata bootstrap` 建立初始 last-known-good，并核对 parts、prices 和 embeddings；采集与发布门禁见[发布管道](../tech/数据获取与发布管道.md)。不要执行 docker compose down -v,除非明确要销毁全部本地数据。
