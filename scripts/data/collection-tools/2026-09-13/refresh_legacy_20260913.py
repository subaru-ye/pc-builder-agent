# -*- coding: utf-8 -*-
"""2026-09-13 旧 160 SKU 价格刷新:用已采集 legacy-price-hits.jsonl(926 行/68 SKU)。

零新增买手调用。判例沿用 v3.1(build_full_update_20260908.cjs):
  - 尾部/【】绑定(tier A):zone 内含全部身份 token 且无其他型号 token → 京东变体选择器行可用
  - 多型号链接显示最低档价不可用:tier B 需全标题无其他型号 token(单型号 listing 才可绑)
  - 尾部命中其他型号变体一律弃行;logical 去重(同价+同规范化标题 = 同一 listing)
  - 晋升必须可复现:决策记录保留 row_key 证据
新增防护(legacy_of 是首命中路由、不感知绑定,这里对每行重算全部 LEGACY_PATTERNS):
  - 品类专属 REQUIRE/FORB 守卫(容量/频率/瓦数/品牌/代际/散装词)
  - CPU 排除板U套装/搭板行;散片与盒装并存时优先盒装
  - 新价偏离旧基线 >±50% → review 默认沿用,人工复核后才可晋升
用法:python refresh_legacy_20260913.py [--emit]  (默认 dry-run 打印逐 SKU 提案)
产出:scripts/data/prices/2026-09-13.csv + catalog-candidates/2026-09-13/legacy-refresh.jsonl
"""
...
ROOT = HERE.parents[3]
HITS = ROOT / "scripts/data/catalog-candidates/2026-09-13/legacy-price-hits.jsonl"
CANDS = ROOT / "scripts/data/catalog-candidates/2026-09-13/candidates.jsonl"
PREV_CSV = ROOT / "scripts/data/prices/2026-09-08.csv"
OUT_CSV = ROOT / "scripts/data/prices/2026-09-13.csv"
OUT_DECISIONS = ROOT / "scripts/data/catalog-candidates/2026-09-13/legacy-refresh.jsonl"
...
JUNK = ["拆机", "二手", "准新", "坏", "维修", "回收", "出租", "样品", "询价", "议价", "成新",
        ...
        "挡板", "理线", "集线器", "电源线", "数据线", "电脑主机", "电竞主机", "游戏主机"]
...
EXTRA_FORB = {
    ...
    "case-fractal-define-7": [r"define7xl"],
...
TAIL_ID: dict[str, list[str]] = {
    ...
    "gpu-asus-4080s-tuf": ["4080s", "4080super"], "gpu-gb-4070-windforce": ["4070"],
    "gpu-gb-5060-windforce": ["5060"], "gpu-gb-5060ti-windforce-16g": ["5060ti"],
    "gpu-gb-5070-windforce-sff": ["5070"], "gpu-gb-5080-gaming": ["5080"],
    "gpu-intel-b580-le": ["b580"], "gpu-msi-3060-ventus2x": ["3060"],
    "gpu-msi-4060-ventus2x-black": ["4060"], "gpu-msi-5070ti-ventus3x": ["5070ti"],
    "gpu-sapphire-7600-pulse": ["7600"], "gpu-sapphire-7700xt-pulse": ["7700xt"],
    ...
    "mem-corsair-veng-32-6000": ["vengeance", "veng", "复仇者"],
    "mem-crucial-ballistix-16-3200": ["ballistix", "铂胜"],
    "mem-gskill-flarex5-32-6000": ["flarex5", "焰锋戟"],
    "mem-gskill-s5-32-6000": ["ripjawss5", "幻锋戟"],
    "mem-kingston-beast-16-6000": ["beast", "野兽"],
    "mem-kingston-beast-32-5600": ["beast", "野兽"],
    ...
    "psu-corsair-rm850x-2021": ["rm850x", "850w"], "psu-msi-mag-a650bn": ["a650bn", "650w"],
...
EXTRA_LITS = {
    ...
    "mem": ["lancer", "lpx", "ripjawsv", "ripjaws", "tridentz5rgb", "tridentz5neo",
            "tridentz5", "幻光戟", "delta", "十铨", "3200", "3600", "5200", "5600", "6000",
            "6400", "6800"],
