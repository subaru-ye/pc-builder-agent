...
新运行记录 `grader_version=independent-v2`：A6 进一步核对本轮实际候选包的对应品类；A10 从冻结规格重新运行兼容规则，拒绝 fail 交付或保存报告与重算不符（veto）；A7 将实际选件报价及已有件标记与冻结价格重算对比。缺少候选/目录等必需证据记 E1。重算复用领域校验引擎，独立于 Harness 的成功声明和缓存结果，不是另一套独立物理规则。旧产物空版本维持历史判卷；同口径重评另列，见[变更回归](变更回归.md)。
...
提示词或源码变更使用 `cmd/evalchange`：默认 plan 零调用，run 构建新程序、使用基线冻结题目并在首个模型调用前检查配置与完整目录；compare 允许二进制不同，但其余条件相同。新旧成绩、同口径复核、实际输出变化及用量分别呈现。调用上限包含模型与 Embedding 的逻辑调用，耗尽后未执行题保留 RUN 失败；逐题落盘，异常退出的半份产物不能通过完整性复验。
