...
网页读取服务启动：`python scripts/start-crawl4ai.py`，再重启 buildsvc。脚本初始化本地凭据并启动 research profile 的固定版本 Docker 容器，不调用模型或搜索。配置、失败行为与补库边界见[网页读取与补库策略](../tech/网页读取与补库策略.md)。
