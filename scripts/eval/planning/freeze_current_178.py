"""Freeze the published snapshot-11 catalog without rerunning intake or changing history."""
...
OUT = ROOT / "artifacts/current-178-freeze-20260917"
CONTAINER = "peval-freeze-178"
...
def main():
    ...
    result = audit.freeze_database(OUT, CONTAINER)
    assert result["latest_id"] == 11
    assert sum(c["active"] for c in result["categories"].values()) == 126
    ...
    assert result["parts_release"].startswith("4dc0da59")
