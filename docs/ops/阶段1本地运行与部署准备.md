...
Harness v2 烟测的 L1/L2 必须来自同一次串行执行，builder 合计调用不超过 6 次；2026-08-29 验收结果为合计 4 次并已默认启用 v2。不允许在同一 run 内自动回退旧链；如需诊断，操作者可显式设置 `BUILD_HARNESS_MODE=legacy`。完整设计与验收数据见 [Agent Harness 2.0](../tech/Agent-Harness-2.0.md)。
