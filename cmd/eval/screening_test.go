...
func TestScreeningRequestsKeepInstructionsAndIsolateSessions(t *testing.T) {
	...
	for i, request := range requests {
		...
		if err := json.Unmarshal(request["instructions"], &instruction); err != nil || !strings.Contains(instruction, "程序已确认") || !strings.Contains(instruction, "游戏名称") || !strings.Contains(instruction, "budget_basis") {
