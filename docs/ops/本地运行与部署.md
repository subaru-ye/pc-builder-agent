...
- 不执行 down -v 作为常规重启。
- RequirementState/RequirementSpec v2 一次性切换(2026-09-22):产品解码边界对 v1 稳定拒绝,不做自动迁移或静默重建。旧开发库中带 v1 需求状态的会话需显式重建——在指定命名空间执行可恢复的清理(例如将目标会话归档后删除其 `web_sessions.requirement_state`/`pending_requirement`,或直接新建会话重新记录),不得在应用启动时批量执行;历史 builds 与分享只读数据不受影响。
