...
> 状态:P7–P9 已实现并完成本地验收;P10 自动门禁与 Live Pass³ 已通过,3 人真人盲评尚未完成。
...
| BUILD_HARNESS_MODE | v2 | builder 执行模式；v2 无工具且最多三次，可显式设为 legacy，不自动回退 |
...
P10 orchestrator 启动前也按上述 service 的 `State/Health` 校验 PostgreSQL 和 Redis。
...
供应商或提示词优化后的最小 L1/L2 单次烟测可额外设置 `P10_SMOKE=1`；它不生成可用于阶段封板的完整报告，也不能代替 Pass³:
