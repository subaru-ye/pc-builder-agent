...
func TestLegacyCannotSilentlySoftenDynamicRequirementStrengths(t *testing.T) {
	for _, draft := range []string{draftJSON, "", "模型正在等待"} {
		...
		v := decide(context.Background(), eval, draft, "", 1, &changeCtx{ActiveSpec: json.RawMessage(`{"schema_version":1,"budget_cny":8000,"use_case":{"type":"general"},"noise_pref":"silent","constraint_strengths":{"noise_pref":"must"}}`)})
