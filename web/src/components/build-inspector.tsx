...
export function BuildInspector({ sessionID, builds, build, latestVersion, canChange, onReplace }: {
  ...
  return <section className="flex h-full min-h-0 flex-col bg-[var(--surface-1)]" aria-label="配置检查器">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto" tabIndex={0} aria-label="检查器内容">
      ...
      {tab === "versions" && <Versions builds={builds} selected={build.summary.version} onSelect={chooseVersion} from={from} to={to} onDiff={setDiff} diff={diff.data} />}
    </div>
  </section>;
}
...
function Versions({ builds, selected, onSelect, from, to, onDiff, diff }: { builds: BuildSummary[]; selected: number; onSelect: (v: number) => void; from: number; to: number; onDiff: (a: number, b: number) => void; diff?: Awaited<ReturnType<typeof api.getDiff>> }) {
