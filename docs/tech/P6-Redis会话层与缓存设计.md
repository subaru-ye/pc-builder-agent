# P6 Redis 会话层与缓存 — 实现设计

> 本文是 **P6 实现层设计**:把 ADK 会话热上下文从进程内 `session.InMemoryService()` 迁到 Redis(带 TTL、两进程共享),让 kill host 重启后同一会话可续；并给 P3 语义检索的 query 向量化加 Redis 缓存。
>
> 口径边界:会话状态即[工程实践指引](工程实践指引.md) §三的状态块(改单版本/预算余量/锁定件),本文不重定义其字段;承接 [P5 设计](P5-A2A单跳拆分设计.md)的双进程拓扑与 contextID 会话映射。开工前对照指引 §九 P6 检查单。冲突时以设计方案 + mvp.md 为准。

## 1. 目标与范围

- **DoD(用例 H,mvp.md §3.7)**:kill 掉 host 进程重启后,同一会话从 Redis 恢复继续改单成功;缓存命中有日志佐证。回归:用例 A/C/D/E/G 不退化。
- **范围内**:新增 `internal/redisstore` 包 —— Redis 客户端拨号 + ADK `session.Service` 的 Redis 实现 + query 向量化缓存装饰器;`cmd/host` 与 `cmd/buildsvc` 注入 Redis 会话服务;buildsvc 的 `QueryEmbedder` 外包一层缓存;`.env` 增 `REDIS_ADDR` / 会话 TTL。
- **范围外(诚实标注)**:**LLM 结果缓存**——`model.LLM.GenerateContent` 是流式 `iter.Seq2`,且本应用每轮请求都带增长的对话历史,多轮请求几乎不重复(命中率约 0),materialize+replay 复杂度换不来开发期成本收益;成本控制沿用已有的**模型分档 + Loop 熔断 + token 预算**(指引 §五.5 也把缓存定位为开发期成本工具)。跨会话长期画像(PRD FR-701)、Redis 集群/持久化策略,均不做。

## 2. 会话持久化边界

P5 双进程各自用进程内 `session.InMemoryService()`:host 会话存对话历史 + 初筛输出 + 远程 `contextID`;buildsvc 会话按 `contextID` 存 `build_state`(改单状态块)。P6 把两侧的 SessionService 都换成同一份 Redis 实现(同一 Redis 实例,key 按 `appName` 天然隔离:host=`pc_builder_host`、buildsvc=`pc_build_service`)。
...
> 关键假设:远程 `contextID` 随 host 会话事件持久(P5 实证「从上一条远程事件元数据复用 contextID」)。本 Redis 实现整条 event 以 JSON 落库,元数据不丢,该假设成立;DoD 步骤 3 专门验证。

## 3. ADK `session.Service` 接线(v2.1.0 实证)

- **要实现的接口**(`session/service.go`,注意名为 `session.Service` 非 `SessionService`,5 方法):
  `Create(ctx,*CreateRequest)`、`Get(ctx,*GetRequest)`、`List(ctx,*ListRequest)`、`Delete(ctx,*DeleteRequest)`、`AppendEvent(ctx, Session, *Event)`。
- **要提供的具体类型**:`Session`/`State`/`Events` 均为接口,需各给一个实现(照 `session/database` 蓝本的 `localSession`/`state`/`events`),并加 `var _ session.Session = (*redisSession)(nil)` 等断言。
- **注入点**:
...
- **List**:用 `SCAN MATCH pcb:sess:{app}:*`(可再按 user 过滤);MVP 规模下 SCAN 足够,且过期键自然不出现,无需维护额外索引集。
...
- key:`pcb:emb:{model}:{sha256(text)}`;value:向量的紧凑编码(JSON `[]float32` 或 little-endian bytes)。命中直接返回,未命中回落底层 client 并写回、设 TTL(`REDIS_CACHE_TTL`,默认 7 天)。
- 日志:命中 `[cache] embedding 命中 model=… len(text)=…`、未命中 `[cache] embedding 未命中 …`(满足检查单「缓存命中有日志」)。
- 正确性:同一模型 + 同一 query 文本 → 同一向量,天然可缓存;查询短文本(软偏好原话/浓缩)在多轮/跨会话高频复现,命中率真实。
- 降级:无 Redis 时装饰器不启用,直接用底层 client(host/buildsvc 均可无 Redis 运行)。

## 6. 进程与配置

- `.env.example` 增:`REDIS_ADDR=localhost:16379`(compose 端口,见 CLAUDE.md)、`REDIS_SESSION_TTL=24h`、`REDIS_CACHE_TTL=168h`。
- go-redis(`github.com/redis/go-redis/v9`)加入依赖(mvp §4.1 已列),`redisstore.Dial(ctx,addr)` 做 `NewClient` + `Ping`。
