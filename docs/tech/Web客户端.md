...
需求编辑器遇到历史固定字段被错误标为 context 时，在用户明确保存后按固定字段的事实或条件分类提交纠正，即使值未变化也可修复；仅打开面板或刷新不写服务端。notes/free.* 的背景分类仍可正常使用。聊天中的错误背景绑定由 Screening 保存为带来源观察，面板更新和聊天更新最终沿用服务端同一 reducer。
