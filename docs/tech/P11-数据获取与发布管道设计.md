...
> 状态：实施中；发布基座和本机调度代码已完成，官方字段适配器与真实周期验收待完成。最后更新：2026-08-24。
>
...
- `00007_p11_data_pipeline.sql`，以及零件与发布记录的 Go 单事务导入。
- Windows 定时任务安装/卸载、隐藏运行、Compose 健康检查和登录后补跑脚本。
- Python 134 项测试；P11 新测试完全使用 fake HTTP，不访问真实来源或模型。
- 已在保留数据卷的真实 PostgreSQL 上迁移到版本 7，完成 160 SKU bootstrap、weekly/monthly 零变化回灌和 run/publication 记录。

尚未完成：

- `amd_products` 只登记为禁用来源；在条款/robots 核验和确定性解析器完成前不会联网。
- 当前定时运行只会验证 seed/锁定来源并得到“无变化”，不会凭空产生新规格或价格。
- 尚未安装用户电脑的计划任务；安装属于工作区外操作，必须由用户显式执行安装脚本。
- 至少一个官方字段适配器、连续两个真实计划周期和 P11 最终 DoD 仍待验收。
...
```json
{
  ...
  "base_url": "https://www.amd.com/en/products/processors.html",
  ...
  "robots_policy": "obey",
  "rate_limit_policy": "low_frequency",
  "schedule": "weekly",
  "enabled": false
}
```
...
- `source_checkpoints` 记录每类计划的 `scheduled_for`、`last_attempt_at` 和 `last_success_at`。
